...
def run_experiments():
    file_full_path = '../test/compare_sort_shift_approach_2025-03-16_1.csv'
    headers = ['workflow', 'shift_mode', 'deadline_factor', 'c-value', 'task_sort', 'use_sort_scheduled', 'makespan',
               'brown_energy_used']
    data_reports = []
    ...
    use_sort_scheduled_parameters = [False, True]
    ...
    for workflow_provider, use_sort_scheduled in itertools.product(workflow_providers, use_sort_scheduled_parameters):

        list_of_makespan = []
        list_of_brown_energy_used = []

        print(f'{workflow_provider[0]} with use_sort_scheduled={use_sort_scheduled}')
        ...
        for shift, d, c, ordering in itertools.product(shift_modes, deadline_factors, c_values,
            ...
            data_reports.append({
                ...
                'c-value': c,
                'task_sort': ordering,
                'use_sort_scheduled': use_sort_scheduled,
                'makespan': makespan,
...
def _create_clusters(graph, deadline_factor):
    ...
    machines = create_machines_with_target(graph, critical_path_length * deadline_factor, [32], 0.80)
    ...
    machines = create_machines_with_target(graph, deadline, [32], 0.80)
